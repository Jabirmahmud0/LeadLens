...
- [ ] `AIProvider` interface: `generate(prompt: string, schema: ZodSchema, options?: AIOptions): Promise<T>`
- [ ] `GeminiProvider` implementation using `@google/generative-ai`
- [ ] `GroqProvider` implementation using `groq-sdk`
- [ ] `withFallback(primary, fallback)` — try primary, fall back to Groq on failure/rate-limit
- [ ] Zod validation of every AI response — retry up to 3x if schema invalid
- [ ] Log every run to `ai_runs` (provider, model, purpose, tokens, latency, retry count, fallback used)
- [ ] Model names from env vars — never hardcoded
...
- [ ] Every factual claim must reference a `sourceUrl` from `source_pages`
- [ ] Inferences labeled `type: 'inference'` — never presented as facts in UI
- [ ] No invention of revenue, headcount, budget, or internal strategy
- [ ] Contact-person claims require public evidence string
- [ ] If data is incomplete, include a limitations object
...
- [ ] After all AI stages complete, save to: `reports`, `report_scores`, `report_findings`, `finding_sources`, `service_recommendations`, `report_outreach`, `report_call_questions`, `report_objections`, `proposal_starters`
- [ ] Report creation is idempotent — use `analysis_job_id` as the dedup key
- [ ] Set `analysis_jobs.status = 'completed'` and `completed_at`
- [ ] Trigger email notification if user opted in
