        ...
        <div className="mx-auto max-w-xl">
          ...
          <form className="mt-8 space-y-5" onSubmit={handleSubmit(onSubmit)} noValidate>
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <div><label className="mb-2 block text-xs font-semibold text-[#365246]" htmlFor="fullName">Full name</label><div className="relative"><UserRound className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-[#789084]" /><input id="fullName" type="text" autoComplete="name" placeholder="Jane Doe" aria-invalid={Boolean(errors.fullName)} className={inputClass} {...register('fullName')} /></div>{errors.fullName && <p className="mt-1.5 text-xs text-rose-600">{errors.fullName.message}</p>}</div>
              <div><label className="mb-2 block text-xs font-semibold text-[#365246]" htmlFor="organizationName">Agency name</label><div className="relative"><UsersRound className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-[#789084]" /><input id="organizationName" type="text" autoComplete="organization" placeholder="Acme Digital" aria-invalid={Boolean(errors.organizationName)} className={inputClass} {...register('organizationName')} /></div>{errors.organizationName && <p className="mt-1.5 text-xs text-rose-600">{errors.organizationName.message}</p>}</div>
            </div>
              <div><label className="mb-2 block text-xs font-semibold text-[#365246]" htmlFor="email">Work email</label><div className="relative"><Mail className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-[#789084]" /><input suppressHydrationWarning id="email" type="email" autoComplete="email" placeholder="jane@agency.com" aria-invalid={Boolean(errors.email)} className={inputClass} {...register('email')} /></div>{errors.email && <p className="mt-1.5 text-xs text-rose-600">{errors.email.message}</p>}</div>
            <div><div className="mb-2 flex items-center justify-between"><label className="text-xs font-semibold text-[#365246]" htmlFor="password">Password</label><span className="text-[10px] font-medium text-[#789084]">12 characters minimum</span></div><div className="relative"><LockKeyhole className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-[#789084]" /><input id="password" type="password" autoComplete="new-password" placeholder="Create a secure password" aria-invalid={Boolean(errors.password)} className={inputClass} {...register('password')} /></div>{errors.password && <p className="mt-1.5 text-xs text-rose-600">{errors.password.message}</p>}</div>
          ...
          <div className="mt-7 flex flex-wrap gap-x-5 gap-y-2 border-t border-[#e0e9e2] pt-6 text-[10px] font-medium text-[#789084]">{['No credit card', 'Public data only', 'Editable output'].map((item) => <span key={item} className="flex items-center gap-1.5"><Check className="size-3.5 text-emerald-600" />{item}</span>)}</div>
        </div>
      </section>
    </div>
  );
