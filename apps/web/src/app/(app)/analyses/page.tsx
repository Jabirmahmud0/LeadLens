...
async function getAnalyses(orgId: string, query: string, status: string, sort: string, page: number) {
  const pageSize = 20;
  const jobs = await db.query.analysisJobs.findMany({
    where: and(
      eq(schema.analysisJobs.organizationId, orgId),
      status ? eq(schema.analysisJobs.status, status) : undefined,
      query ? sql`exists (select 1 from ${schema.prospects} p where p.id = ${schema.analysisJobs.prospectId} and (p.company_name ilike ${`%${query}%`} or p.normalized_domain ilike ${`%${query}%`}))` : undefined,
    ),
    orderBy: [sort === 'oldest' ? asc(schema.analysisJobs.createdAt) : desc(schema.analysisJobs.createdAt)],
    with: { prospect: true },
    limit: pageSize,
...
export default async function AnalysesPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; sort?: string; page?: string }> }) {
  ...
  const analyses = await getAnalyses(session.organization.id, query, status, sort, page);
    ...
    <div className="app-page-enter mx-auto flex min-h-full max-w-6xl flex-col space-y-6 p-4 sm:p-7 lg:p-9">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {a.status === 'completed' ? (
                        ...
                        <a
                          href={`/analyses/${a.id}`}
                          className="flex items-center gap-1.5 px-4 py-2 border border-neutral-300 bg-transparent text-neutral-600 hover:border-neutral-400 hover:text-neutral-800 rounded-xl text-[12px] font-bold transition-all shrink-0"
                        >
                          View Status
                        </a>
                      )}
                      <AnalysisRowActions id={a.id} />
                    </div>
                  </div>
                </div>
              </div>
      ...
      {(page > 1 || analyses.length === 20) && (
        <nav className="flex justify-center gap-3 text-sm">
          {page > 1 && <a className="rounded-lg border border-neutral-300 px-4 py-2 text-neutral-600 hover:border-neutral-400" href={`/analyses?q=${encodeURIComponent(query)}&status=${encodeURIComponent(status)}&sort=${sort}&page=${page - 1}`}>Previous</a>}
          {analyses.length === 20 && <a className="rounded-lg border border-neutral-300 px-4 py-2 text-neutral-600 hover:border-neutral-400" href={`/analyses?q=${encodeURIComponent(query)}&status=${encodeURIComponent(status)}&sort=${sort}&page=${page + 1}`}>Next</a>}
        </nav>
      )}
    </div>
  );
